...
export function ConflictDrawer({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex">
      ...
      <aside className="w-full max-w-md overflow-auto border-l border-slate-800 bg-slate-900 text-slate-100 shadow-2xl shadow-signal/10">
        <header className="flex items-center justify-between border-b border-slate-800 p-4">
          <h2 className="text-lg font-semibold text-slate-100">Offline queue</h2>
          <button
            onClick={onClose}
            className="text-sm text-slate-400 hover:text-cyan-100 hover:underline"
          >
            Close
          </button>
        </header>
